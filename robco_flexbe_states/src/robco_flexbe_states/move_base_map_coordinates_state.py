...
class MoveBaseMapCoordinatesState(EventState):
    """
    Navigates a robot to a desired position and orientation (in map coordinates) using move_base.
    ...
    def __init__(self):
        ...
        super(MoveBaseMapCoordinatesState, self).__init__(outcomes = ['arrived', 'failed'],
                                            input_keys = ['waypoint'])
    ...
    def on_enter(self, userdata):
        ...
        #################################
        # SRG Stefan
        # Get point coordinates in the map by clicking Publish point in Rviz and hovering over 
        # the map (do not click the map) down below on the screen you'll see the coordinates 
        # of the point on the map (x-coord,y-coord,0) /ommit the third number/
